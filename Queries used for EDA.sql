...
/*Revenue and DAU(Daily Active Users) each day during data period (Revenue number was rounded up
to 2 digits after decimal point)*/
SELECT date,
...
/*Mean of Levels Played per day in three periods: Before Event, During Event, After
Event*/
...
GROUP BY date;/* ARPDAU (average revenue per daily active user), ARPDAU were rounded up to 3
...
ORDER BY date;/* ARPPU Average Revenue Per Paying User: average revenue per user who spent
something on that, the numbers were rounded up to 2 digits after decimal point*/
SELECT date,
	ROUND(SUM(dbo.Assignment.revenue_that_day) / COUNT(CASE WHEN dbo.Assignment.revenue_that_day > 0 THEN 1 END),2) AS ARPPU
FROM dbo.Assignment
GROUP BY date
ORDER BY date;/*The days since installation (Age) of each user, their lifetime revenue, the total levels
they played*/
...
FROM dbo.Assignment
GROUP BY id;/*lifetime revenue, lifetime revenue per user, levels played, levels played per user
categorized by 5 different Age Groups: 0-50, 51-100, 101-150, 151-200, >200*/
